...
pub async fn run(task_str:Option<&str>, config_ptr:Arc<serde_yaml::Value>) -> bool{
    match task_str {
        ...
        Some("masking") => masking::masking_runner::run(config_ptr, MaskType::Mlm).await,
        Some("causal") => masking::masking_runner::run(config_ptr, MaskType::Causal).await,
        Some("t5") => masking::masking_runner::run(config_ptr, MaskType::Span).await,
